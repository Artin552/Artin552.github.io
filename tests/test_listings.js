...
// Run from project root: node tests/test_listings.js
...
(async ()=>{
  ...
  try{
    ...
    console.log('created', r.status, created);
    const id = created.id;

    // 4) get mine
    r = await fetch(base + '/api/listings?mine=true', { headers:{ Authorization: 'Bearer ' + token } });
    const mine = await r.json();
    console.log('mine count', mine.length);

    // 5) update
    r = await fetch(base + '/api/listings/' + id, { method:'PUT', headers:{'Content-Type':'application/json', Authorization: 'Bearer ' + token}, body: JSON.stringify({ price:'2500' }) });
    const updated = await r.json();
    console.log('updated', r.status, updated);

    // 6) delete
    r = await fetch(base + '/api/listings/' + id, { method:'DELETE', headers:{ Authorization: 'Bearer ' + token } });
    const del = await r.json();
    console.log('deleted', r.status, del);
